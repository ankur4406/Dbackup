...
#    rotated = cv2.warpAffine(form_gray, M, (w, h), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)
    ...
    # Replace the variable sections with individual fields # Change 2
    dict_config = repeat_sections(dict_config, form_gray, scale_large, scale_small, out_dir) # Change 2
    ...
    for key in dict_config.keys():
        ...
        slc = misc.toimage(form_gray[y1:y2, x1:x2]) # 1 change
...
def repeat_sections(config, form, scale_large, scale_small, out_dir): # 3 change
    ...
    for item in sec_list:
        ...
        print("template match scores are {} and {}".format(max_val1, max_val2))
        ...
        print (y2, y1, scale_large, sec_count)
        ...
        slc = misc.toimage(form[y1:y2, x1:x2])
        misc.imsave(os.path.join(out_dir, item + '_slice.png'), slc)
